...
parser.add_argument('--dataset', default='NELL', help='')

parser.add_argument('--rel_str_embed', default=300, type=int, help='relation structural embeddings size')
parser.add_argument('--ent_str_embed', default=300, type=int, help='entity structural embeddings size')
parser.add_argument('--ent_text_embed', default=300, type=int, help='entity textual embeddings size')
parser.add_argument('--mapping_size', default=300, type=int, help='hidden layer size')
...
entity_text_embeddings_file = os.path.join(param.datadir, param.dataset, 'onto_file', 'embeddings',
                                           'Onto_Text_Embed.pkl')
...
entity_embeddings_img = u.load_binary_file(
    entity_text_embeddings_file)  # "multimodal/fb_vgg128_avg_fb_txt_normalized.pkl"
...
with tf.name_scope('head_relation'):
    # structure
    r_mapped = my_dense(r_input, param.mapping_size, activation_fn=param.activation_function, scope="txt_proj",
                        reuse=None)
    r_mapped = tf.nn.dropout(r_mapped, keep_prob)

    h_pos_txt_mapped = my_dense(h_pos_txt_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="txt_proj", reuse=True)
    h_pos_txt_mapped = tf.nn.dropout(h_pos_txt_mapped, keep_prob)

    h_neg_txt_mapped = my_dense(h_neg_txt_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="txt_proj", reuse=True)
    h_neg_txt_mapped = tf.nn.dropout(h_neg_txt_mapped, keep_prob)

    t_pos_txt_mapped = my_dense(t_pos_txt_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="txt_proj", reuse=True)
    t_pos_txt_mapped = tf.nn.dropout(t_pos_txt_mapped, keep_prob)

    t_neg_txt_mapped = my_dense(t_neg_txt_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="txt_proj", reuse=True)
    t_neg_txt_mapped = tf.nn.dropout(t_neg_txt_mapped, keep_prob)

    # Visual
    h_pos_img_mapped = my_dense(h_pos_img_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="img_proj", reuse=None)
    h_pos_img_mapped = tf.nn.dropout(h_pos_img_mapped, keep_prob)

    h_neg_img_mapped = my_dense(h_neg_img_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="img_proj", reuse=True)
    h_neg_img_mapped = tf.nn.dropout(h_neg_img_mapped, keep_prob)

    # Tail image ....
    t_pos_img_mapped = my_dense(t_pos_img_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="img_proj", reuse=True)
    t_pos_img_mapped = tf.nn.dropout(t_pos_img_mapped, keep_prob)

    t_neg_img_mapped = my_dense(t_neg_img_input, param.mapping_size, activation_fn=param.activation_function,
                                scope="img_proj", reuse=True)
    t_neg_img_mapped = tf.nn.dropout(t_neg_img_mapped, keep_prob)

with tf.name_scope('cosine'):
    # Head model
    energy_ss_pos = tf.reduce_sum(abs(h_pos_txt_mapped + r_mapped - t_pos_txt_mapped), 1, keep_dims=True,
                                  name="pos_s_s")
    energy_ss_neg = tf.reduce_sum(abs(h_pos_txt_mapped + r_mapped - t_neg_txt_mapped), 1, keep_dims=True,
                                  name="neg_s_s")

    energy_is_pos = tf.reduce_sum(abs(h_pos_img_mapped + r_mapped - t_pos_txt_mapped), 1, keep_dims=True,
                                  name="pos_i_i")
    energy_is_neg = tf.reduce_sum(abs(h_pos_img_mapped + r_mapped - t_neg_txt_mapped), 1, keep_dims=True,
                                  name="neg_i_i")

    energy_si_pos = tf.reduce_sum(abs(h_pos_txt_mapped + r_mapped - t_pos_img_mapped), 1, keep_dims=True,
                                  name="pos_s_i")
    energy_si_neg = tf.reduce_sum(abs(h_pos_txt_mapped + r_mapped - t_neg_img_mapped), 1, keep_dims=True,
                                  name="neg_s_i")

    energy_ii_pos = tf.reduce_sum(abs(h_pos_img_mapped + r_mapped - t_pos_img_mapped), 1, keep_dims=True,
                                  name="pos_i_i")
    energy_ii_neg = tf.reduce_sum(abs(h_pos_img_mapped + r_mapped - t_neg_img_mapped), 1, keep_dims=True,
                                  name="neg_i_i")

    energy_concat_pos = tf.reduce_sum(
        abs((h_pos_txt_mapped + h_pos_img_mapped) + r_mapped - (t_pos_txt_mapped + t_pos_img_mapped)), 1,
        keep_dims=True, name="energy_concat_pos")
    energy_concat_neg = tf.reduce_sum(
        abs((h_pos_txt_mapped + h_pos_img_mapped) + r_mapped - (t_neg_txt_mapped + t_neg_img_mapped)), 1,
        keep_dims=True, name="energy_concat_neg")

    h_r_t_pos = tf.reduce_sum([energy_ss_pos, energy_is_pos, energy_si_pos, energy_ii_pos, energy_concat_pos], 0,
                              name="h_r_t_pos")
    h_r_t_neg = tf.reduce_sum([energy_ss_neg, energy_is_neg, energy_si_neg, energy_ii_neg, energy_concat_neg], 0,
                              name="h_r_t_neg")

    # Tail model

    score_t_t_pos = tf.reduce_sum(abs(t_pos_txt_mapped - r_mapped - h_pos_txt_mapped), 1, keep_dims=True,
                                  name="pos_s_s")
    score_t_t_neg = tf.reduce_sum(abs(t_pos_txt_mapped - r_mapped - h_neg_txt_mapped), 1, keep_dims=True,
                                  name="neg_s_s")

    score_i_t_pos = tf.reduce_sum(abs(t_pos_img_mapped - r_mapped - h_pos_txt_mapped), 1, keep_dims=True,
                                  name="pos_i_i")
    score_i_t_neg = tf.reduce_sum(abs(t_pos_img_mapped - r_mapped - h_neg_txt_mapped), 1, keep_dims=True,
                                  name="neg_i_i")

    score_t_i_pos = tf.reduce_sum(abs(t_pos_txt_mapped - r_mapped - h_pos_img_mapped), 1, keep_dims=True,
                                  name="pos_s_i")
    score_t_i_neg = tf.reduce_sum(abs(t_pos_txt_mapped - r_mapped - h_neg_img_mapped), 1, keep_dims=True,
                                  name="neg_s_i")

    score_i_i_pos = tf.reduce_sum(abs(t_pos_img_mapped - r_mapped - h_pos_img_mapped), 1, keep_dims=True,
                                  name="pos_i_i")
    ...
    energy_concat_pos_tail = tf.reduce_sum(
        abs((t_pos_txt_mapped + t_pos_img_mapped) - r_mapped - (h_pos_txt_mapped + h_pos_img_mapped)), 1,
    ...
    t_r_h_pos = tf.reduce_sum([score_t_t_pos, score_i_t_pos, score_t_i_pos, score_i_i_pos, energy_concat_pos_tail], 0,
                              name="t_r_h_pos")
    t_r_h_neg = tf.reduce_sum([score_t_t_neg, score_i_t_neg, score_t_i_neg, score_i_i_neg, energy_concat_neg_tail], 0,
                              name="t_r_h_neg")
...
with tf.Session(config=sess_config) as sess:
    ...
    for epoch in range(param.training_epochs):
        ...
        if (epoch + 1) >= 50 and (epoch + 1) % 10 == 0:
            ...
            if param.dataset == 'NELL':
                rel2id = json.load(open(DATASET_DIR + '/relation2ids'))
            if param.dataset == 'Wiki':
                rel2id = json.load(open(DATASET_DIR + '/relation2ids_1'))
            ...
            save_name = 'rela_matrix_onto_' + str((epoch + 1)) + '.npz'
